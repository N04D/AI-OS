...
def load_or_init_budget_state(
    *,
    ...
    for key in state["counts"]:
        value = ((loaded.get("counts") or {}).get(key))
        if isinstance(value, int) and value >= 0:
            state["counts"][key] = value
        value = ((loaded.get("last_action_epoch_s") or {}).get(key))
        if isinstance(value, int) and value >= 0:
            state["last_action_epoch_s"][key] = value
        value = ((loaded.get("last_consume_keys") or {}).get(key))
        if isinstance(value, str):
            state["last_consume_keys"][key] = value
    return state, resolved_state
